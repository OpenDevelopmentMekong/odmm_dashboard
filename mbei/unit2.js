function drawMap() {
  ...
  areas.selectAll('path')
    ...
    .attr('fill', '#110000')
    .attr('stroke', '#aabbcc')
    .attr('onclick', function(d) { var tafName=pcodeToTAF(d.properties[accessorForPcode]); return `drawSubNationChart("` + tafName + `", false, '.unit2RadarChart'); d3.select('#subnationalDropdown').property('value', '` + tafName + `');`;})
